...
def observe_system_state(tasks, u_big, u_little, channel_gain, min_vals, max_vals):
    # State: (SU, U_big, U_little, WCET, B, h)
...
if __name__ == '__main__':
    ...
    min_state_vals = np.array([0, 0, 0, 2, 150, 0], dtype=float)
    max_state_vals = np.array([1, 1, 1, 10, 500, 2*w_inter.cg_sigma], dtype=float)
    ...
    next_tasks = tg.step(100)
    next_states = observe_system_state(next_tasks,
                                       cpu_big.util, cpu_little.util,
                                       cg, min_state_vals, max_state_vals)
    all_rewards = []
    all_penalties = []
    all_min_penalties = []
    for itr in range(NUM_ITR):
        ...
        # Execute tasks
        cpu_little.step(tasks, actions['little'])
        cpu_big.step(tasks, actions['big'])
