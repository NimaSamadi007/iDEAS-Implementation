...
def evaluate_task_size_scenario(configs, task_size_val, eval_itr=10000):
    ...
    # Evaluate the trained model
    #print("Evaluating the trained model...")
    dqn_task_energy_cons = np.zeros((4,len(task_size_val)-1))
    dqn_num_tasks = np.zeros((4,len(task_size_val)-1))
    rrlo_task_energy_cons = np.zeros((4,len(task_size_val)-1))
    rrlo_num_tasks = np.zeros((4,len(task_size_val)-1))
    #conference_task_energy_cons = np.zeros((4,len(task_size_val)-1))
    #conference_num_tasks = np.zeros((4,len(task_size_val)-1))

    local_task_energy_cons = np.zeros((4,len(task_size_val)-1))
    local_num_tasks = np.zeros((4,len(task_size_val)-1))
    remote_task_energy_cons = np.zeros((4,len(task_size_val)-1))
    remote_num_tasks = np.zeros((4,len(task_size_val)-1))
    random_task_energy_cons = np.zeros((4,len(task_size_val)-1))
    random_num_tasks = np.zeros((4,len(task_size_val)-1))

    local_task_deadline_missed = np.zeros((4,len(task_size_val)-1))
    random_task_deadline_missed = np.zeros((4,len(task_size_val)-1))
    remote_task_deadline_missed = np.zeros((4,len(task_size_val)-1))
    dqn_task_deadline_missed = np.zeros((4,len(task_size_val)-1))
    rrlo_task_deadline_missed = np.zeros((4,len(task_size_val)-1))
    #conference_task_deadline_missed = np.zeros((4,len(task_size_val)-1))
    ...
    for i in range(len(task_size_val)-1):
        ...
        #tasks = task_gen.step(target_cpu_load,task_size_val[2*i])
        #print(task_size_val[2*i])
        dqn_state, _ = dqn_env.observe(copy.deepcopy(tasks))
        rrlo_state, _ = rrlo_env.observe(copy.deepcopy(tasks))
        #conference_state, _ = conference_env.observe(copy.deepcopy(tasks))
        local_env.observe(copy.deepcopy(tasks))
