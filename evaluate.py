...
def iDEAS_evaluate(
    ...
    if taskset_eval:
        ...
        avg_dqn_little_energy = np.sum(little_energy, axis=0) / np.sum(
            dqn_num_tasks, axis=0
        )
        avg_dqn_big_energy = np.sum(big_energy, axis=0) / np.sum(dqn_num_tasks, axis=0)
        avg_dqn_offload_energy = np.sum(offload_energy, axis=0) / np.sum(
            dqn_num_tasks, axis=0
        )
    ...
    if CPU_load_eval:
        ...
        avg_big_cpu_energy = np.sum(big_cpu_energy, axis=0) / np.sum(
            dqn_cpu_num_tasks, axis=0
        )
        avg_little_cpu_energy = np.sum(little_cpu_energy, axis=0) / np.sum(
            dqn_cpu_num_tasks, axis=0
        )
        avg_offload_cpu_energy = np.sum(offload_cpu_energy, axis=0) / np.sum(
            dqn_cpu_num_tasks, axis=0
        )

        total_dqn_missed_cpu = (
            np.sum(dqn_cpu_deadline_missed, axis=0)
            / np.sum(dqn_cpu_num_tasks, axis=0)
            * 100
        )
        total_big_missed_cpu = (
            np.sum(big_cpu_deadline_missed, axis=0)
            / np.sum(dqn_cpu_num_tasks, axis=0)
            * 100
        )
        total_little_missed_cpu = (
            np.sum(little_cpu_deadline_missed, axis=0)
            / np.sum(dqn_cpu_num_tasks, axis=0)
            * 100
        )
        total_offload_missed_cpu = (
            np.sum(offload_cpu_deadline_missed, axis=0)
            / np.sum(dqn_cpu_num_tasks, axis=0)
            * 100
        )
    ...
    if task_size_eval:
        ...
        for i in range(len(task_sizes) - 1):
            ...
            for _ in range(eval_itr):
                ...
                if i == len(task_sizes) - 2:
                    tasks = normal_task_gen.step(
                        target_cpu_load, task_sizes[i], max_task_load
                    )
                else:
                    tasks = normal_task_gen.step(
        ...
        avg_big_task_energy = np.sum(big_task_energy, axis=0) / np.sum(
            dqn_task_num_tasks, axis=0
        )
        avg_little_task_energy = np.sum(little_task_energy, axis=0) / np.sum(
            dqn_task_num_tasks, axis=0
        )
        avg_offload_task_energy = np.sum(offload_task_energy, axis=0) / np.sum(
            dqn_task_num_tasks, axis=0
        )

        total_dqn_missed_task = (
            np.sum(dqn_task_deadline_missed, axis=0)
            / np.sum(dqn_task_num_tasks, axis=0)
            * 100
        )

        total_big_missed_task = (
            np.sum(big_task_deadline_missed, axis=0)
            / np.sum(dqn_task_num_tasks, axis=0)
            * 100
        )

        total_little_missed_task = (
            np.sum(little_task_deadline_missed, axis=0)
            / np.sum(dqn_task_num_tasks, axis=0)
            * 100
        )

        total_offload_missed_task = (
            np.sum(offload_task_deadline_missed, axis=0)
            / np.sum(dqn_task_num_tasks, axis=0)
            * 100
        )
    ...
    if CN_eval:
        ...
        avg_big_cn_energy = np.sum(big_cn_energy, axis=0) / np.sum(
            dqn_cn_num_tasks, axis=0
        )
        avg_little_cn_energy = np.sum(little_cn_energy, axis=0) / np.sum(
            dqn_cn_num_tasks, axis=0
        )
        avg_offload_cn_energy = np.sum(offload_cn_energy, axis=0) / np.sum(
            dqn_cn_num_tasks, axis=0
        )

        total_dqn_missed_cn = (
            np.sum(dqn_cn_deadline_missed, axis=0)
            / np.sum(dqn_cn_num_tasks, axis=0)
            * 100
        )
        total_big_missed_cn = (
            np.sum(big_cn_deadline_missed, axis=0)
            / np.sum(dqn_cn_num_tasks, axis=0)
            * 100
        )
        total_little_missed_cn = (
            np.sum(little_cn_deadline_missed, axis=0)
            / np.sum(dqn_cn_num_tasks, axis=0)
            * 100
        )
        total_offload_missed_cn = (
            np.sum(offload_cn_deadline_missed, axis=0)
            / np.sum(dqn_cn_num_tasks, axis=0)
            * 100
        )
...
def RRLO_evaluate(
    ...
    if taskset_eval:
        ...
        avg_local_energy = np.sum(local_energy, axis=0) / np.sum(
            local_num_tasks, axis=0
        )
        avg_remote_energy = np.sum(remote_energy, axis=0) / np.sum(
            remote_num_tasks, axis=0
        )
        avg_random_energy = np.sum(random_energy, axis=0) / np.sum(
            random_num_tasks, axis=0
        )
        ...
        total_local_missed = (
            np.sum(local_deadline_missed, axis=0)
            / np.sum(local_num_tasks, axis=0)
            * 100
        )
        total_remote_missed = (
            np.sum(remote_deadline_missed, axis=0)
            / np.sum(remote_num_tasks, axis=0)
            * 100
        )
        total_random_missed = (
            np.sum(random_deadline_missed, axis=0)
            / np.sum(random_num_tasks, axis=0)
            * 100
        )
        total_rrlo_missed = (
    ...
    if CPU_load_eval:
        ...
        avg_local_energy_cpu = np.sum(local_cpu_energy, axis=0) / np.sum(
            local_cpu_num_tasks, axis=0
        )
        avg_remote_energy_cpu = np.sum(remote_cpu_energy, axis=0) / np.sum(
            remote_cpu_num_tasks, axis=0
        )
        avg_random_energy_cpu = np.sum(random_cpu_energy, axis=0) / np.sum(
            random_cpu_num_tasks, axis=0
        )
        avg_rrlo_energy_cpu = np.sum(rrlo_cpu_energy, axis=0) / np.sum(
            rrlo_cpu_num_tasks, axis=0
        )
        ...
        total_local_missed_cpu = (
            np.sum(local_cpu_deadline_missed, axis=0)
            / np.sum(local_cpu_num_tasks, axis=0)
            * 100
        )
        total_remote_missed_cpu = (
            np.sum(remote_cpu_deadline_missed, axis=0)
            / np.sum(remote_cpu_num_tasks, axis=0)
            * 100
        )
        total_random_missed_cpu = (
            np.sum(random_cpu_deadline_missed, axis=0)
            / np.sum(random_cpu_num_tasks, axis=0)
            * 100
        )
        total_rrlo_missed_cpu = (
            np.sum(rrlo_cpu_deadline_missed, axis=0)
            / np.sum(rrlo_cpu_num_tasks, axis=0)
            * 100
        )
    ...
    if task_size_eval:
        ...
        avg_local_energy_task = np.sum(local_task_energy, axis=0) / np.sum(
            local_task_num_tasks, axis=0
        )
        avg_remote_energy_task = np.sum(remote_task_energy, axis=0) / np.sum(
            remote_task_num_tasks, axis=0
        )
        avg_random_energy_task = np.sum(random_task_energy, axis=0) / np.sum(
            random_task_num_tasks, axis=0
        )
        avg_rrlo_energy_task = np.sum(rrlo_task_energy, axis=0) / np.sum(
            rrlo_task_num_tasks, axis=0
        )
        ...
        total_local_missed_task = (
            np.sum(local_task_deadline_missed, axis=0)
            / np.sum(local_task_num_tasks, axis=0)
            * 100
        )
        total_remote_missed_task = (
            np.sum(remote_task_deadline_missed, axis=0)
            / np.sum(remote_task_num_tasks, axis=0)
            * 100
        )
        total_random_missed_task = (
            np.sum(random_task_deadline_missed, axis=0)
            / np.sum(random_task_num_tasks, axis=0)
            * 100
        )
        total_rrlo_missed_task = (
            np.sum(rrlo_task_deadline_missed, axis=0)
            / np.sum(rrlo_task_num_tasks, axis=0)
            * 100
        )
    ...
    if CN_eval:
        ...
        avg_rrlo_cn_energy = np.sum(rrlo_cn_energy, axis=0) / np.sum(
            rrlo_cn_num_tasks, axis=0
        )
        avg_local_cn_energy = np.sum(local_cn_energy, axis=0) / np.sum(
            local_cn_num_tasks, axis=0
        )
        avg_remote_cn_energy = np.sum(remote_cn_energy, axis=0) / np.sum(
            remote_cn_num_tasks, axis=0
        )
        avg_random_cn_energy = np.sum(random_cn_energy, axis=0) / np.sum(
            random_cn_num_tasks, axis=0
        )
        ...
        total_rrlo_missed_cn = (
            np.sum(rrlo_cn_deadline_missed, axis=0)
            / np.sum(rrlo_cn_num_tasks, axis=0)
            * 100
        )
        total_local_missed_cn = (
            np.sum(local_cn_deadline_missed, axis=0)
            / np.sum(local_cn_num_tasks, axis=0)
            * 100
        )
        total_remote_missed_cn = (
            np.sum(remote_cn_deadline_missed, axis=0)
            / np.sum(remote_cn_num_tasks, axis=0)
            * 100
        )
        total_random_missed_cn = (
            np.sum(random_cn_deadline_missed, axis=0)
            / np.sum(random_cn_num_tasks, axis=0)
            * 100
        )
    ...
    if taskset_eval:
        for i in range(2):
            ...
            for _ in range(eval_itr):
                ...
                actions_random_str = RandomDQNPolicyGen(
                    rand_littlefreq_list, rand_bigfreq_list, rand_powers_list
                )
        ...
        results["taskset_energy"] = np.array(
            [avg_random_energy, avg_dqn_energy, avg_local_energy, avg_remote_energy]
        )
        ...
        results["taskset_improvement"] = np.array(
            [dqn_random_improvement, dqn_local_improvement, dqn_remote_improvement]
        )
...
def RandomPolicyGen(freqs, powers):
    ...
    actions["local"] = [
        [i + offload, freqs[idx]] for i, idx in enumerate(random_freq_idx)
    ]
...
def RandomDQNPolicyGen(littlefreqs, bigfreqs, powers):
    ...
    random_littlefreq_idx = np.random.choice(
        len(littlefreqs), size=little, replace=True
    )
    random_bigfreq_idx = np.random.choice(len(bigfreqs), size=big, replace=True)
    random_power_idx = np.random.choice(len(powers), size=offload, replace=True)
    actions["offload"] = [[i, powers[idx]] for i, idx in enumerate(random_power_idx)]
    actions["little"] = [
        [i + offload, littlefreqs[idx]] for i, idx in enumerate(random_littlefreq_idx)
    ]
    actions["big"] = [
        [i + offload + little, bigfreqs[idx]]
